...
def history():
	...
	try:
		db = get_db()
		cursor = db.cursor()
		results = cursor.execute("""SELECT * from (SELECT rowid as "..",Method,URL, PARAMS, BODY as "Request body", STATUS FROM "requests" order BY rowid DESC limit 10) order by ".." ASC""").fetchall()
		if results == []:
...
def show_index_from_history(index):
	...
	try:
		db = get_db()
		cursor = db.cursor()
		cursor.execute(f"""SELECT * FROM "requests" WHERE rowid in ({index})""")
	...
	print(f"Params"+ " " * (column_1_length+4-len('Params')), "" if result_index['Params'] is None else result_index['Params'])
	print(f"Headers"+ " " * (column_1_length+4-len('Headers')), "" if result_index['Headers'] is None else result_index['Headers'])
	print(f"Request Body"+ " " * (column_1_length+4-len('Request Body')), "" if result_index['Body'] is None else result_index['Body'])
	print(f"Basic Authentication"+ " " * (column_1_length+4-len('Basic Authentication')), "" if result_index['Auth'] is None else result_index['Auth'])
